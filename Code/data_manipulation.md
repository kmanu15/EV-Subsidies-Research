...
```
    ...
    ## [1] "/Users/kojomanu/Desktop/UO/Oregon Classes/EC 418-419"
...
Editing model names so that I can combine this data set with the one I
made. This is because tax credit differs based on range and energy type,
but the model name is the same for all
...
Changing CHEVROLET model names to differentiate tax credit amount
depending on purchase date (using `DOL.Transaction.Date` as an
approximation)
...
write.csv(merged_data, "WA_FIPS.csv", row.names = F)
```

Changing `X2020.Census.Tract` so that it’s the same format as census
data sets
...
``` r
# Saving Data Set
write.csv(merged_data, "WA_EV_data.csv", row.names = FALSE)
```

Commute Time Census Data
...
write.csv(Commute_Time, "Commute_TimeF.csv", row.names = F)
```
...
write.csv(Census_Income, "Census_IncomeF.csv", row.names = F)
```
